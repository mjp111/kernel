			 ...
			 struct hmm_vma_walk *hmm_vma_walk, unsigned long cpu_flags)
{
	...
	if (migrate && thp_migration_supported() &&
	    ...
	    IS_ALIGNED(end, HPAGE_PMD_SIZE) &&
		 ...
		 if ((range->hmm_pfns[i] & (HMM_PFN_MIGRATE | HMM_PFN_VALID)) !=
		     (HMM_PFN_MIGRATE | HMM_PFN_VALID)) {
			      ...
			      pmd_t pmd)
{
	...
	for (i = 0; addr < end; addr += PAGE_SIZE, i++, pfn++) {
		if ((hmm_pfns[i] & (HMM_PFN_MIGRATE | HMM_PFN_VALID)) !=
                    (HMM_PFN_MIGRATE | HMM_PFN_VALID)) {
...
out:
	if ((*hmm_pfn & (HMM_PFN_MIGRATE | HMM_PFN_VALID)) !=
	    (HMM_PFN_MIGRATE | HMM_PFN_VALID))
					      ...
					      unsigned long *hmm_pfn)
{
	...
	for (i = 0, tmp = start; tmp < end; tmp += PAGE_SIZE, i++) {
		if ((hmm_pfn[i] & (HMM_PFN_VALID | HMM_PFN_MIGRATE)) ==
...
again:
	...
	if (thp_migration_supported() && pmd_is_migration_entry(pmd)) {
		...
		for (i = 0; start < end; start += PAGE_SIZE, i++) 
			if ((hmm_pfns_start[i] & (HMM_PFN_MIGRATE | HMM_PFN_VALID)) !=
			    (HMM_PFN_MIGRATE | HMM_PFN_VALID))
