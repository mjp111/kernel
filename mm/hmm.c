			      ...
			      pmd_t pmd)
{
	...
	hmm_vma_handle_migrate_prepare_rollback(hmm_vma_walk, addr, end, hmm_pfns);
				     ...
				     pmd_t pmd)
{
	...
	if (softleaf_is_device_private(entry) &&
		 ...
		 */
		hmm_vma_handle_migrate_prepare_rollback(hmm_vma_walk, start, end, hmm_pfns);
...
again:
	...
	if (thp_migration_supported() && pmd_is_migration_entry(pmd)) {
		...
		hmm_vma_handle_migrate_prepare_rollback(walk->private, addr, end, hmm_pfns_start);
		for (i = 0; start < end; start += PAGE_SIZE, i++)
