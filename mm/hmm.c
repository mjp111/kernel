...
struct hmm_vma_walk {
	...
	/* Set once mmu_range is armed with mmu_notifier_invalidate_range_start() */
	bool				mmu_armed;
 ...
 * page tables.
 *
 * Safe to reuse hmm_pfns[] as collected-state because migrate mode drives
 * fault/migration requests only via range->default_flags; per-PFN request
 * flags are unsupported. Since REQ_FAULT/REQ_MIGRATE alias VALID/MIGRATE,
 * this is what guarantees those bits appear only from a real collection,
 * never as caller input -- so hmm_pfn_collected() cannot false-positive, and
 * clearing them here cannot destroy a request (default_flags is re-derived on
 * every re-walk). VALID is cleared (not just MIGRATE) because rollback drops
 * the folio reference, and VALID must not outlive the pin.
 */
static void hmm_vma_handle_migrate_prepare_rollback(const struct hmm_vma_walk *hmm_vma_walk,
						    unsigned long start,
						    unsigned long end,
	...
	do {
		...
		if (ret == HMM_FAULT_PENDING) {
			...
			if (ret == HMM_FAULT_UNLOCKED) {
				ret = fatal_signal_pending(current) ? -EINTR :
					-EBUSY;
				break;
			}
		}
